...
SESSION_METRICS = {
    "total_packets": 0,
    "network_types": {"Local Loopback": 0, "Private LAN": 0, "Attacks 🚀💥": 0, "Invalid/Unknown": 0},
...
def print_summary():
    ...
    print("=" * 65)
    print(f" 🛰️  Total Probes Dispatched:  {SESSION_METRICS['total_packets']}")
...
def main():
    ...
    print("\n🛠️ [CONFIG ENGINE] Adjust parameters or press ENTER for Infinity parameters:")

    packet_input = input(" ▫️ Packets to stream [Default: ♾️]: ").strip()
    infinite_packets = False if packet_input else True
    total_packets = int(packet_input) if not infinite_packets else 0

    thread_input = input(" ▫️ Concurrent thread pipes [Default: Max OS Threading]: ").strip()
    threads = int(thread_input) if thread_input else None

    timeout_input = input(" ▫️ Timeout limit (Seconds) [Default: ♾️/None]: ").strip()
    timeout = float(timeout_input) if timeout_input else None

    print("\n📡 " + "-" * 61 + " 📡")
    print(f" 🌐 Target Endpoint:   {scheme}://{host}:{port}{path} ({target_ip})")
    print(f" 🎚️ Network Layer:    {net_type}")
...
if __name__ == "__main__":
    try:
        ...
        print("\n💥 [FORCED ABORT] Hard break intercepted. System RAM caches dropped immediately.")
        sys.exit()
